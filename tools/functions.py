...
def load_state_dict_distill(model, ckpt_file, only_base='no', without_fc='no'):
    print('\n\n\n load_distill')
    src_state_dict = torch.load(ckpt_file, map_location=lambda storage, loc: storage)
    #target_dict_name_list = list(model.state_dict().keys())
    #base_dict_name_list = [p for n in target_dict_name_list if n.startswith('base')]
    ...
    for name, param in src_state_dict.items():
        ...
        try:
            ...
            if name.startswith('base.layer3.'):
                dest_state_dict[name.replace('base.layer3.', 'base.3.')].copy_(param)
                continue
            if name.startswith('base.layer4.'):
                dest_state_dict[name.replace('base.layer4.', 'base_branch1.0.')].copy_(param)
                dest_state_dict[name.replace('base.layer4.', 'base_branch1.0.')].copy_(param)
                continue
            if name.startswith('bottleneck'):
                dest_state_dict[name.replace('bottleneck', 'bottleneck_branch1')].copy_(param)
                dest_state_dict[name.replace('bottleneck', 'bottleneck_branch2')].copy_(param)
                continue
